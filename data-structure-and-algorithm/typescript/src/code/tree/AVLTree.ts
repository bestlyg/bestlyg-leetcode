...
export default class AVLTree<T> extends BinarySearchTree<T> {
  ...
  private rebalance(
    grand: Node<T>,
    method: rebalanceMethodType = "unified"
  ): void {
    switch (method) {
      case "unified":
        this.rebalanceUnified(grand);
      case "common":
        this.rebalanceCommon(grand);
    }
  }
  private rebalanceCommon(grand: Node<T>): void {
  ...
  private rebalanceUnified(grand: Node<T>): void {
    const parent = (grand as AVLNode<T>).tallerChild();
    const node = (parent as AVLNode<T>).tallerChild();
    if (parent.isLeftChild()) {
      if (node.isLeftChild()) {
  ...
  private rotateLeft(grand: Node<T>): void {
    ...
    const child: Node<T> = parent.left!;
    grand.right = parent;
    parent.left = grand;
  ...
  private rotateRight(grand: Node<T>): void {
    ...
    const child: Node<T> = parent.right!;
    grand.left = child;
    parent.right = grand;
    this.afterRotate(grand, parent, child);
  }
  private afterRotate(grand: Node<T>, parent: Node<T>, child: Node<T>) {
